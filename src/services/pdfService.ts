...
export const generateOrderPDF = (order: Order): string => {
  ...
  doc.setFontSize(10);
  doc.text("Dia chi: 123 Duong ABC, Quan XYZ, TP.HCM", 105, 30, {
    align: "center",
  });
  ...
  doc.text(
    `Ngay: ${new Date(order.createdAt).toLocaleDateString("vi-VN")}`,
    20,
    55
  );
  doc.text(
    `Gio: ${new Date(order.createdAt).toLocaleTimeString("vi-VN")}`,
    20,
    60
  );
  ...
  doc.text(
    `Khach hang: ${removeVietnameseAccents(order.customerName)}`,
    120,
    50
  );
  ...
  if (order.discount) {
    ...
    doc.text(formatCurrency(order.discount), 190, finalY + 5, {
      align: "right",
    });
  }
  ...
  doc.text("Tong cong:", 130, finalY + 15);
  doc.text(formatCurrency(order.finalAmount), 190, finalY + 15, {
    align: "right",
  });
  ...
  doc.text(
    `Phuong thuc thanh toan: ${getPaymentMethodName(order.paymentMethod)}`,
    20,
    finalY + 25
  );
  doc.text(
    `Trang thai thanh toan: ${getPaymentStatusName(order.paymentStatus)}`,
    20,
    finalY + 30
  );
  ...
  // Thêm ghi chú nếu có
  if (order.note) {
    doc.text("Ghi chu:", 20, finalY + 40);
    doc.text(removeVietnameseAccents(order.note), 20, finalY + 45);
  }

  // Thêm lời cảm ơn
  doc.setFontSize(10);
  doc.text("Cam on quy khach da su dung dich vu!", 105, finalY + 55, {
    align: "center",
  });
...
export const downloadOrderPDF = (order: Order): void => {
  ...
  // Tải xuống PDF
  doc.save(`Hoa_don_${order.orderNumber}.pdf`);
};
...
export const generateCartPDF = (
  ...
  const tempOrder: Order = {
    id: "temp-" + Date.now(),
    orderNumber: orderNumber || "TEMP-" + Date.now().toString().slice(-6),
